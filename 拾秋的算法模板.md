...
inline int ls(int p) { return p << 1; }     //左儿子 p*2
inline int rs(int p) { return p << 1 | 1; } //右儿子 p*2+1
...
void build(int p, int pl, int pr){           //建树。p为节点编号，它指向区间[pl, pr]
...
void addtag(int p, int pl, int pr, ll d){    //给节点p打tag标记，并更新tree
    tag[p] += d;                             //打上tag标记
    tree[p] += d * (pr - pl + 1);            //计算新的tree值
}

void push_down(int p, int pl, int pr){       //不能覆盖时，把tag传给子树
...
void update(int L, int R, int p, int pl, int pr, ll d){ //区间修改：[L,R]内每个元素加d
...
ll query(int L, int R, int p, int pl, int pr){
    ...
    if(pl > R || pr < L) return tree[p];    //完全覆盖，直接返回
